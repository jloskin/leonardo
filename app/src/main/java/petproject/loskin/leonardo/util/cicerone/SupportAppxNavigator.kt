...
class SupportAppxNavigator(
    ...
    protected fun activityForward(command: Forward) {
        val screen = command.screen as SupportAppxScreen
    ...
    protected fun fragmentForward(command: Forward) {
        ...
        setupFragmentTransaction(
                command,
                fragmentManager.findFragmentById(containerId) ?: Fragment(),
                fragment,
                fragmentTransaction
        )
    ...
    protected fun backTo(command: BackTo) {
        if (command.screen == null) {
            ...
            if (index != -1) {
                ...
                backToUnexisting(command.screen as SupportAppxScreen)
            }
        }
    }
    ...
    private fun checkAndStartActivity(screen: SupportAppxScreen, activityIntent: Intent, options: Bundle?) {
     ...
     */
    protected fun unexistingActivity(screen: SupportAppxScreen, activityIntent: Intent) {
     ...
     */
    protected fun createFragment(screen: SupportAppxScreen): Fragment = screen.fragment
     ...
     * @param screen screen
     */
    protected fun backToUnexisting(screen: SupportAppxScreen) {
        backToRoot()
    }
}